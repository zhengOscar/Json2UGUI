...
#region LoopScrollRect
        ...
        //[NonSerialized]
        //[HideInInspector]
        //public IScrollerDelegate Delegate = null;

        System.Func<LoopScrollRect, int> NumberOfCells;
        System.Func<LoopScrollRect, int, string> NameOfCellPrefab;
        System.Func<LoopScrollRect, string> BundleNameOfCellPrefab;
        System.Func<LoopScrollRect, string> PathOfCellPrefab;
        System.Action<GameObject> OnCellCreate;
        System.Func<ItemRenderer,int, int> OnCellVisiable;
        System.Func<ItemRenderer,int, int> OnSelectedCellHandler;
        ...
        public void SetHandler(System.Func<LoopScrollRect, int> NumberOfCells,
                        System.Func<LoopScrollRect, int, string> NameOfCellPrefab,
                        System.Func<LoopScrollRect, string> BundleNameOfCellPrefab,
                        System.Func<LoopScrollRect, string> PathOfCellPrefab,
                        System.Action<GameObject> OnCellCreate,
                        System.Func<ItemRenderer, int, int> OnCellVisiable,
                        System.Func<ItemRenderer, int, int> OnSelectedCell)
        {
            this.NumberOfCells = NumberOfCells;
            this.NameOfCellPrefab = NameOfCellPrefab;
            this.BundleNameOfCellPrefab = BundleNameOfCellPrefab;
            this.PathOfCellPrefab = PathOfCellPrefab;
            this.OnCellCreate = OnCellCreate;
            this.OnCellVisiable = OnCellVisiable;
            this.OnSelectedCellHandler = OnSelectedCell;
        }
...
#region LoopScrollRect
        ...
        public ItemRenderer GetCellOfIndex(int index)
        {
            if (index >= 0 && index < totalCount)
            {
                ItemRenderer[] items = content.GetComponentsInChildren<ItemRenderer>();
        ...
        public void ClearCells()
        {
            if (Application.isPlaying)
            {
                ...
                {
                    ReturnItemToPool(content.GetChild(i));
                }
            }
        }

        public bool ScrollToCell(int index, float speed)
        {
            if (!Application.isPlaying || !isActiveAndEnabled)
                return false;

            totalCount = GetNumberOfCells();
        ...
        /// <summary>
        /// speed固定移动距离（帧长变化时，移动距离不变）
        /// 帧长变化时原来的方法在第一次启动unity执行时会出现定位不到的情况
        /// </summary>
        /// <param name="index"></param>
        /// <param name="speed"></param>
        /// <returns></returns>
        public bool ScrollToCellFixSpeed(int index, float speed)
        {
            if (!Application.isPlaying || !isActiveAndEnabled)
                return false;
            ...
            StopAllCoroutines();
            StartCoroutine(ScrollToFixCellCoroutine(index, speed));
            return true;
        }

        IEnumerator ScrollToFixCellCoroutine(int index, float speed)
        {
            bool needMoving = true;
            while (needMoving)
            {
        ...
        /// 新增刷新数据
        /// </summary>
        public void CheckNewAdd(bool isRefillCellsFromEnd = false)
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                int detal = GetNumberOfCells() - totalCount;
                //有新增
                if (detal > 0)
                {
                    totalCount = GetNumberOfCells();
		...
		public void Refresh(bool isRefillCellsFromEnd = false){
			//刷新添加
            CheckNewAdd(isRefillCellsFromEnd);
            ...
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < content.childCount; i++)
                {
                    ...
                    {
                        //m_DataSource.ProvideData(content.GetChild(i), itemTypeEnd);
                        if (OnCellVisiable != null)
                        {
                            OnCellVisiable(content.GetChild(i).GetComponent<ItemRenderer>(), itemTypeEnd);
                        }
                        itemTypeEnd++;
                    }
                    else
        ...
        public void RefillCellsFromEnd(int offset = 0)
        {
            ...
            {
                ReturnItemToPool(m_Content.GetChild(i));
            }
        ...
        protected float NewItemAtStart()
        {
            ...
            if (loop)
            {
                if (itemTypeStart == 0) itemTypeStart = totalCount;
            }
            #endregion
        ...
        protected float DeleteItemAtStart()
        {
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                ...
                if (loop && itemTypeStart==totalCount)
                {
                    itemTypeStart = 0;
                }
        ...
        private RectTransform InstantiateNextItem(int itemIdx)
        {
            ...
            if (BundleNameOfCellPrefab != null)
                bundleName = BundleNameOfCellPrefab(this);
            if (NameOfCellPrefab != null)
                prefabName = NameOfCellPrefab(this, itemIdx);
            if (PathOfCellPrefab != null)
                prefabPath = PathOfCellPrefab(this);
            
            prefabSource.InitSource(bundleName, prefabName, prefabPath, OnCellCreate);
            
            ItemRenderer nextItem = prefabSource.GetObject(prefabName).GetComponent<ItemRenderer>();
            nextItem.cellIndex = itemIdx;
            nextItem.OnSelected = OnSelectedCell;
            nextItem.transform.SetParent(content, false);
            nextItem.gameObject.SetActive(true);
            if (OnCellVisiable != null)
            {
                OnCellVisiable(nextItem, itemIdx);
            }

            return nextItem.GetComponent<RectTransform>();
        }

        private void ReturnItemToPool(Transform item)
        {
            prefabSource.ReturnObject(item);

            ItemRenderer cell = item.GetComponent<ItemRenderer>();
            if (cell != null)
            {
        ...
        public void Release()
        {
            prefabSource.Release();
            prefabSource = null;
        }

        private void OnSelectedCell(ItemRenderer cell, int cellIndex)
        {
            if (this.OnSelectedCellHandler != null)
            {
                OnSelectedCellHandler(cell, cellIndex);
            }
        }
#endregion
